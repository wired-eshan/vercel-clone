...
async function deleteFolder(bucketName, folderId) {
  ...
  try {
    do {
      ...
      console.log("List of objects with prefix: ", listedObjects);
...
router.post("/upload", authMiddleware, async (req, res) => {
  ...
  const existingDeployment = await prisma.deployment.findFirst({
    where: {
      ...
      OR: [{ status: "QUEUED" }, { status: "BUILDING" }],
    },
  });
  if (existingDeployment) {
    return res
      .status(400)
      .json({ error: "Project is already queued for build" });
  }
  ...
  const command = new RunTaskCommand({
    ...
    networkConfiguration: {
      awsvpcConfiguration: {
        subnets: [
          "subnet-056b58ac45618cfad",
          "subnet-0a816b169578d08d8",
          "subnet-03b3f6ebdb35a5b2b",
        ],
...
router.delete("/:projectId", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    //delete project files from AWS S3
    const projectDirectory = `__outputs/${projectId}`;
    await deleteFolder(process.env.S3_BUCKET, projectDirectory);
